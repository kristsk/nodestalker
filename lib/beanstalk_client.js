...
BeanstalkClient.prototype.command = function(obj) {
	...
	var dataHandler = function(data) {
		...
		else if(firstLineChunks[0] == "RESERVED")
		{
			if((lines[0].length + 2 + Number(firstLineChunks[2]) + 2) == Number(data.length))
			{
				this.currentPendingResponseBufferPosition = 0;
			...
			else
			{
				...
				this.pendingResponseBuffer = new Buffer(lines[0].length + 2 + Number(firstLineChunks[2]) + 2);
